...
- Acesso a plataforma da Advents.
...
<Warning>Cada um dos pacotes adicionais pode necessitar de configurações específicas.</Warning>

## 2. Inicialização

Após a instalação, você precisa inicializar o SDK com sua chave de API. Recomendamos fazer isso o mais cedo possível no ciclo de vida do seu aplicativo, preferencialmente no componente de entrada do seu app.
...
Após finalizar a instalação e inicialização, entre em contato com a nossa equipe através do [suporte](/suporte) para verificar se os eventos enviados pela SDK estão corretos.
